...
def wait_till_no_xfers(rpc_client: libs.pyboinc.rpc_client) -> None:
    ...
    while current_loops < max_loops:
        ...
        if isinstance(allow_response, list):
            log.error(
                "Unexpected response1 in wait_till_no_xfers: " + str(allow_response)
            )
            ...
            if cleaned_response == "":  # There are no transfers, yay!
                ...
                log.error(
                    "Unexpected response2 in wait_till_no_xfers: "
                    + str(cleaned_response)
                )
        log.error(
            "Unexpected response3 in wait_till_no_xfers: " + str(cleaned_response)
        )
