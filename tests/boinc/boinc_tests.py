...
def test_get_attached_projects(test_setup_connection):
    result1,result2=loop.run_until_complete(main.get_attached_projects(RPC_CONNECTION_OBJECT))
...
def test_verify_boinc_connection(test_setup_connection):
    result1=loop.run_until_complete(main.verify_boinc_connection(RPC_CONNECTION_OBJECT))
    assert result1
